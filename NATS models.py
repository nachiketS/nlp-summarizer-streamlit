...
class AttentionEncoder(nn.Module):
    ...
    def __init__(self,src_hidden_size,trg_hidden_size,attn_method,repetition):
        ...
        if self.method == 'luong_concat':
            self.attn_en_in = nn.Linear(self.src_hidden_size,
                                        self.trg_hidden_size,
                                        bias=True)
            self.attn_de_in = nn.Linear(self.trg_hidden_size,
                                              self.trg_hidden_size,
                                              bias=False)
            self.attn_cv_in = nn.Linear(1, self.trg_hidden_size, bias=False)
            self.attn_warp_in = nn.Linear(self.trg_hidden_size, 1, bias=False)
        if self.method == 'luong_general':
            self.attn_in = nn.Linear(self.src_hidden_size,
                                           self.trg_hidden_size,
                                           bias=False)
...
class AttentionDecoder(nn.Module):
    ...
    def __init__(self, hidden_size, attn_method):
        ...
        if self.method == 'luong_concat':
            self.attn_en_in = nn.Linear(self.hidden_size,
                                        self.hidden_size,
                                        bias=True)
            self.attn_de_in = nn.Linear(self.hidden_size,
                                              self.hidden_size,
                                              bias=False)
            self.attn_warp_in = nn.Linear(self.hidden_size, 1, bias=False)
        if self.method == 'luong_general':
            self.attn_in = nn.Linear(self.hidden_size, 
                                           self.hidden_size,
                                           bias=False)
...
class LSTMDecoder(nn.Module):
    def __init__(
        ...
        if self.pointer_net:
            if self.attn_decoder:   
                self.pt_out = nn.Linear(
                    self.input_size+self.src_hidden_size+self.trg_hidden_size*2, 1)
            else:
                self.pt_out = nn.Linear(
                    self.input_size+self.src_hidden_size+self.trg_hidden_size, 1)
    ...
    def forward(
        ...
        loss_cv = Variable(torch.zeros(1))
        batch_size = input_.size(1)
            ...
            if self.attn_decoder:
                if k + idx == 0:
                    c_decoder = Variable(torch.zeros(
                        batch_size, self.trg_hidden_size))
                else:
...
class Seq2Seq(nn.Module):
    ...
    def __init__(
        ...
        if self.shared_emb:
            ...
            self.src_embedding = nn.Embedding(
                self.src_vocab_size,
                self.src_emb_dim)
            nn.init.uniform_(self.src_embedding.weight, -1.0, 1.0)
            self.trg_embedding = nn.Embedding(
                self.trg_vocab_size,
                self.trg_emb_dim)
            nn.init.uniform_(self.trg_embedding.weight, -1.0, 1.0)
        # network structure
        ...
        if self.share_emb_weight:
            self.decoder2proj = nn.Linear(
                self.trg_hidden_dim,
                self.src_emb_dim,
                bias=False)
            self.proj2vocab = nn.Linear(
                self.src_emb_dim,
                self.trg_vocab_size,
                bias=True)
    ...
    def forward(self, input_src, input_trg):
        ...
        h0_encoder = Variable(torch.zeros(
            ...
            batch_size, self.src_hidden_dim))
        if self.repetition == 'temporal':
        ...
        h_attn = Variable(torch.zeros(
            batch_size, self.trg_hidden_dim))
        p_gen = Variable(torch.zeros(
            batch_size, trg_seq_len))
        past_dehy = Variable(torch.zeros(1, 1))
        # network
        if self.network_ == 'lstm':
    ...
    def forward_onestep_decoder(
        ...
        # pointer weight
        p_gen = Variable(torch.zeros(batch_size, 1))
        # decoder
        if self.network_ == 'lstm':
    ...
    def cal_dist(self, input_src, logits_, attn_, p_gen, src_vocab2id):
        ...
        pt_idx = Variable(torch.FloatTensor(torch.zeros(1, 1, 1)))
    ...
    def cal_dist_explicit(self, input_src, logits_, attn_, p_gen, vocab2id, ext_id2oov):
        ...
        logits_ex = Variable(torch.zeros(1, 1, 1))
